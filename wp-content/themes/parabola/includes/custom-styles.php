...
for($i=1;$i<=6;$i++):
	...
	echo round(($font_root-($i*0.27))*(preg_replace("/[^\d]/","",$parabola_headingsfontsize)/100),4);
	echo "em;} ";
	echo "#frontpage h$i { font-size: ";
	echo round(($font_root-0.5-($i*0.3))*(preg_replace("/[^\d]/","",$parabola_headingsfontsize)/100),4);
	echo "em;} ";
endfor; ?>
...
function parabola_customcss() {
	...
	if ($parabolas['parabola_customcss'] != "") {
		echo '<style type="text/css">' . htmlspecialchars_decode( $parabolas['parabola_customcss'], ENT_QUOTES ) . '</style>';
	}
} // parabola_customcss()
