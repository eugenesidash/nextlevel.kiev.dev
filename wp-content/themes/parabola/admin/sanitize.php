...
function parabola_settings_validate($input) {
	...
	$colorSchemes = ( ! empty( $input['parabola_schemessubmit']) ? true : false ) && ( isset($parabola_colorschemes_array[$input['parabola_colorschemes']]) );
	if ($colorSchemes) {
		$input = array_merge( $parabolas, json_decode( "{" . $parabola_colorschemes_array[$input['parabola_colorschemes']] . "}", true ) );
		return $input;
	};
